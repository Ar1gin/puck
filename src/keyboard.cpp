...
void Keyboard::buttonClicked(int key) {
  if (key != 0) {
	...
	bool alt = key & 0x00040000;
	bool meta = key & 0x00080000;
    ...
    if (alt) {
      emit_code(keyboard_fd, EV_KEY, KEY_LEFTALT, 1);
    }
    if (meta) {
      emit_code(keyboard_fd, EV_KEY, KEY_LEFTMETA, 1);
    }
    ...
    if (alt) {
      emit_code(keyboard_fd, EV_KEY, KEY_LEFTALT, 0);
    }
    if (meta) {
      emit_code(keyboard_fd, EV_KEY, KEY_LEFTMETA, 0);
    }
